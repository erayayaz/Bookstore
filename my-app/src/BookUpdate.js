...
class BookUpdate extends React.Component {
  ...
  handleSubmit = (e) => {
    ...
    if (publisher.length < 3 || publisher.length > 50) {
      this.setState({
        publisherError: "Invalid publisher",
      });
      return;
    }

    fetch(`http://localhost:8081/api/admins/update_book/${this.state.bookId}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
      },
      ...
      .then((response) => {
        toast.success(
          "Şifre yenileme işlemi başarılı giriş sayfasını yönlendiriliyorsunuz."
        );
      })
      .catch((e) => {
        toast.error(e.message);
  ...
  render = () => {
    const { publisherError } = this.state;
    return (
      <div className="App">
        <Container>
          <Grid>
