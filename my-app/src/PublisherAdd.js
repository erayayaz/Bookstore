...
class PublisherAdd extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      name: "",
  ...
  handleSubmit = (e) => {
    ...
    if (phone.length < 7 || phone.length > 11) {
      this.setState({
        phoneError: "İnvalid phone",
      });
      return;
    }

    fetch("http://localhost:8081/api/admins/add_publisher", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  render = () => {
    const { nameError, addressError, phoneError } = this.state;
    return (
      <div className="App">
        <Container>
          <Grid>
...
export default withRouter(PublisherAdd);
