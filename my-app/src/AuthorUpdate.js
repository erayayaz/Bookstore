...
class AuthorUpdate extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      authorId: 0,
  ...
  handleSubmit = (e) => {
    ...
    if (surname.length < 2 || surname.length > 50) {
      this.setState({
        surnameError: "Lütfen soyismini kontrol ediniz.",
      });
      return;
    }

    fetch(
      `http://localhost:8081/api/admins/update_author/${this.state.authorId}`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
      ...
      .then((r) => r.json())
      .then((response) => {
        toast.success("Yazar soyadı yenileme işlemi başarılı...");
      })
      .catch((e) => {
        toast.error(e.message);
      });
...
export default withRouter(AuthorUpdate);
