...
class UserSearch extends React.Component {
  ...
  searchUsers = () => {
    fetch(
      `http://localhost:8081/api/admins/search_user?username=${this.state.username}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
      ...
      .then((data) => {
        ...
        toast.success("Search process is successful");
            ...
            <Grid.Row columns="equal" centered>
              <Grid.Column width={16}>
                <Table celled>
                  ...
                  <Table.Body>
                    {users && users.createDate && (
                      <Table.Row>
                        <Table.Cell>{users.id}</Table.Cell>
                        <Table.Cell>{users.username}</Table.Cell>
                        <Table.Cell>{users.createDate.toString()}</Table.Cell>
                      </Table.Row>
                    )}
                  </Table.Body>
